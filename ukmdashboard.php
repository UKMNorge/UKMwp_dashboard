...
function UKMwpd_menu() {
	if( in_array( get_option('site_type'), array('kommune','fylke','land')) ) {
		...
		$page = add_menu_page('Stimuleringsmidler', 'Stimuleringsmidler', 'editor', 'UKMstimulering', 'UKMstimulering', '//ico.ukm.no/cash-menu.png',5);
...
function UKMWP_dash_scriptsandstyles() {
	...
	if( $screen->base == 'dashboard' || $screen->base == 'dashboard-network' ) {
		...
		if( $screen->base == 'dashboard' ) {
            ...
            wp_enqueue_style('UKMWPdash_tribute_css', plugin_dir_url(__FILE__) .'/css/tribute.css');
            wp_enqueue_script('UKMWPdash_tribute_js', plugin_dir_url(__FILE__) .'/js/tribute.min.js');
		}
